...
new_check_tails <- function(nanopolish,
  ...
  tryCatch({
    ...
    tail_features_list <- foreach::foreach(i = seq_along(polya_summary$readname),
                                           ...
                                           .export = "new_extract_tail_data",            # export function to workers
                                           .packages = c("rhdf5","ninetails","checkmate","assertthat")) %dopar% {lapply(polya_summary$readname[i], function(x) new_extract_tail_data(x,polya_summary,workspace,basecall_group))}
    
    
    close(pb)
    
    testing_tail_features_list <<- tail_features_list
...
new_extract_tail_data <- function(readname,
                                  polya_summary,
                                  workspace,
                                  basecall_group){
  #Assertions
  ...
  # [CHANGE MADE IN THE FOLLOWING LINE]
  ...
  if (is.null(called_events) || is.na(called_events) || !is.numeric(called_events))
    stop("called_events missing or invalid for read ", readname)
  if (is.null(stride) || is.na(stride) || !is.numeric(stride))
    stop("block_stride missing or invalid for read ", readname)
